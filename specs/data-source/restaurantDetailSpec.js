...
import RestoDataSource from '../../src/scripts/data/restaurant-data-source';
  ...
  describe('When response status ok', () =>{
    describe('valid JSON response', () => {
      it('returns restaurant detail', async () => {
        ...
        const response = await RestoDataSource.getRestaurantDetail('123');
        expect(response)
      ...
      it('should throw error', async () => {
        ...
        try {
          ...
          expect(error instanceof Error)
              .toEqual(true);
          expect(error.message)
              .toEqual('An error occurred while processing data.');
        }
      });
    });
  });

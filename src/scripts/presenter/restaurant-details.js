...
class RestaurantDetailPresenter extends Presenter {
  ...
  async _addToFavorite(restaurant) {
    ...
    this._view.showSnackBar('Added to favorites');

    if (process.env.NODE_ENV === 'development') {
      getElement('restaurant-details').dispatchEvent(new Event('snackbar:updated'));
    }
  }

  async _removeFromFavorite(id) {
    await this._model.favorite.deleteRestaurant(id);
    this._view.showSnackBar('Removed from favorites');
